var PlayersCollection = Backbone.Collection.extend({
  ...
  url : function() {
    ...
    // console.log('url
    // Players',appConfig.serviceURLPlayers+'autocomplete/?q='+this.query);
  ...
  /*
   * follow: function(follow) { // FIXME : 1 objet if
   * (window.localStorage.getItem("PlayersFollow")!==null ) { //on cree la pile
   * var pile = new
   * Players({url:JSON.parse(window.localStorage.getItem("PlayersFollow"))});
   * pile.add(follow) ;
   * window.localStorage.setItem("PlayersFollow",JSON.stringify(pile)); }
   * //Ajoute le premier element else
  ...
  /*
   * comparator: function(item) { //POSSIBLE MULTI FILTER [a,b,..] return
   * [item.get("city")]; },
   */
