...
var GameModel = Backbone.Model.extend({
   ...
   * 
   * console.log('method sync Model Game',method); //console.log('method
   * playerid ',this.get('playerid'));
   * 
   * if ( method === 'create' && this.get('playerid')!==undefined ) {
   * 
   * 
   * var team1_json=''; var team2_json='';
   * 
   * //if player exists / not exists
   * 
   * if (this.get('team1_id') === '' ) team1_json =
   * {name:this.get('team1'),rank:'NC'}; else team1_json =
   * {id:this.get('team1_id')};
   * 
   * if (this.get('team2_id') === '' ) team2_json =
   * {name:this.get('team2'),rank:'NC'}; else team2_json =
   * {id:this.get('team2_id')};
   * 
   * 
   * var object = { teams:[{id:null,players:[team1_json]},
   ...
   * model.url()+'?playerid='+(this.get('playerid') ||
   * '')+'&token='+(this.get('token') || ''), type:'POST', data:object, success:
   * function (result) { // put your code after the game is saved/updated.
   * 
   ...
   *  // si erreur on stocke dans localstorage console.log('error Game',result);
   * 
   * 
   * 
   *  } });
   ...
   * return $.ajax({ dataType: 'json', url:
   * model.url()+'?playerid='+(this.get('playerid') ||
   * '')+'&token='+(this.get('token') || ''), type:'POST', data: { //FIXME : BUG
   * player_id au lieu de owner //playerid: (this.get('playerid') || ''),
   * //token: (this.get('token') || ''),
   * 
   * //teams:[{id:null,players:[{id:this.get('team1_id')}]}, //
   * {id:null,players:[{id:this.get('team2_id')}]}], //integrer les points
   * options:{sets: (this.get('sets') || '')} }, success: function (result) { //
  ...
  /*
   * toJSON: function () { var json =
   * Backbone.Model.prototype.toJSON.call(this); json.date =
